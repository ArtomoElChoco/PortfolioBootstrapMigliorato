...
const tutteLeCard = document.querySelectorAll(".card")
const tuttiITitoli = document.querySelectorAll(".card-title")
const tuttiITesti = document.querySelectorAll(".card-text")
const tuttiIProsegui = document.querySelectorAll(".prosegui")
...
function CambiaColore(){
    if(this.style.backgroundColor == ""){
        ...
        tutteLeCard.forEach(card => card.style.backgroundColor = "#ffffff")
        tuttiITitoli.forEach(titolo => titolo.style.color = "#2a2860")
        tuttiITesti.forEach(testo => testo.style.color = "#000000")
        tuttiIProsegui.forEach(p => p.style.color = "#000000")
    }
    else{
        localStorage.setItem("colore", "dark")
        ...
        tutteLeCard.forEach(card => card.style.backgroundColor = "")
        tuttiITitoli.forEach(titolo => titolo.style.color = "")
        tuttiITesti.forEach(testo => testo.style.color = "")
        tuttiIProsegui.forEach(p => p.style.color = "")
    }
}
